 ...
 */
package com.intellij.openapi.diff.impl.collab;
...
public class ApplyPatchDifferentiatedDialog extends DialogWrapper {
    ...
    private class MyUpdater implements Runnable {
        public void run() {
            ...
            final List<FilePatchInProgress> matchedPathes = new MatchPatchPaths(myProject).execute(patchReader.getPatches());
    ...
    private PatchReader loadPatches(final FilePresentation filePresentation) {
        ...
        PatchReader reader;
        try {
            final byte[] patchContents = patchFile.contentsToByteArray();
            final CharSequence patchText = LoadTextUtil.getTextByBinaryPresentation(patchContents, patchFile);
            reader = new PatchReader(patchText);
        } catch (IOException e) {
            return null;
        }
        try {
